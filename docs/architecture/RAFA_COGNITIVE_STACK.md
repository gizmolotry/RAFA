...
To prevent gradient starvation and "brain-dead" synthesizers, RAFA follows a non-linear curriculum designed to force functional distribution across the stack.

### Phase 0 — Pretrain the Voice (Competence, not Cleverness)
**Goal:** The denoiser must map structured latents back to plausible audio before it is asked to "think."
* **Objective:** Local realism, timbre fidelity, transient accuracy.
* **Constraints:** Freeze all semantic/router parts. Train solely on reconstruction.
* **Mantra:** Teach the mouth to pronounce before the system to think.

### Phase 1 — Train the Spine (Long-Horizon Persistence)
**Goal:** Train Hyena, Clutches, and temporal transport to maintain structural thread.
* **Objective:** Hold rhythm, envelope, and trajectory across long windows.
* **Tasks:** Masked-span reconstruction, macro-envelope prediction from partial context.
* **Mantra:** Don't drop the civilization.

### Phase 2 — Train the Brain (Policy Isolation)
**Goal:** Force the Impedance Head, Router, and Slow Clock to become a control policy.
* **Objective:** Infer macro-state, decide q-band gates, respond to proprioceptive feedback.
* **Constraints:** Train in isolation on state/control tasks (e.g., matching Tension Targets).
* **Mantra:** Govern, don't memorize.

### Phase 3 — Constitutional Binding (Full Stack)
**Goal:** Joint end-to-end training under a multi-loss constitution.
* **A. Micro (Voice):** Diffusion denoising and fine phase accuracy.
* **B. Macro (Spine):** Envelope consistency and beat continuity.
* **C. Brain (Policy):** q-band selection consistency and slow-state forecasting.
* **D. Binding (Alignment):** Enforce conditional dependence of the renderer on the router state.

### The Invariance Curriculum (The Missing Ingredient)
The model must learn to separate **Texture** from **State**. 
* **Batch Type 1:** Same structure (e.g., Airplane Tension), different timbre textures.
* **Batch Type 2:** Same timbre (e.g., White Noise), different macro-state (Cruising vs. Takeoff).
* **Objective:** Force the slow latent to stay identical across timbral variants while allowing micro-latents to diverge.

## 9. Implementation Plan (Blackwell sm_120 DAG)

The training is executed via a modular 4-stage DAG to prevent feature smuggling and shortcut learning.

### Stage 1: Voice Boot Camp (The Mouth)
* **Script:** `tools/train_stage1_voice.py`
* **Goal:** Denoising competence and timbre fidelity.
* **Status:** [READY] 

### Stage 2: Spine Boot Camp (The Skeleton)
* **Script:** `tools/train_stage2_spine.py`
* **Goal:** Long-range structural persistence via masked spans.
* **Status:** [READY]

### Stage 3: Brain Boot Camp (The Policy)
* **Script:** `tools/train_stage3_brain.py` (Successor to `train_functional.py`)
* **Goal:** Latent alignment to tension targets. 
* **Status:** [ACTIVE - PID 9892]

### Stage 4: Constitutional Binding (The Stack)
* **Script:** `tools/train_stage4_joint.py`
* **Goal:** Full joint finetuning under the Stacked Loss Constitution.
* **Status:** [READY]
