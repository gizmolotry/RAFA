...
class NakedDenoiser:
    ...
    def forward(self, xt_mag, xt_z, t, rafa_mag=None, rafa_z=None):
        ...
        if rafa_mag is not None and rafa_z is not None:
            guide = torch.stack([rafa_mag, rafa_z[..., 0], rafa_z[..., 1]], dim=1)
            guide = (guide - guide.mean(dim=(2,3), keepdim=True)) / (guide.std(dim=(2,3), keepdim=True) + 1e-6)
            guide_flat = guide.permute(0, 2, 3, 1)
            guide_feat = F.linear(guide_flat, self.weights["cond_proj_w"].view(128, 3), self.weights["cond_proj_b"]).permute(0, 3, 1, 2)
            gate = torch.sigmoid(self.weights["cond_gate_w"].view(1, 128, 1, 1))
            h = h + gate * guide_feat
